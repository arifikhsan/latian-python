...
def main():
    ...
    employee_with_date = download_file(FILE_URL)
    reader = csv.reader(employee_with_date[1:])
    employee_with_date_list = list(reader)
    employee_with_date_list.sort(key=lambda x: x[3])
    list_newer(employee_with_date_list, start_date)
